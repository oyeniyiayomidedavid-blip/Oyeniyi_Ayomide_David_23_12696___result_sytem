...
// Fetch results pending approval
try {
    $stmt = $pdo->query("
        ...
        WHERE r.approved = 0 AND r.rejected = 0
        ORDER BY r.id DESC
    ");
...
<div class="sidebar">
    ...
    <a href="result_approval.php" class="active"><i class="fa fa-check-circle"></i> Result Approval</a>
...
<div class="content">
    ...
    <p>Below is a list of all unapproved results. You can approve or reject them using the buttons.</p>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php if(count($pendingResults) > 0): ?>
                    <?php foreach($pendingResults as $res): ?>
                        <tr>
                            <td><?= htmlspecialchars($res['full_name']) ?></td>
                            <td><?= htmlspecialchars($res['matric_no']) ?></td>
                            <td><?= htmlspecialchars($res['course_code']) ?></td>
                            <td><?= htmlspecialchars($res['course_title']) ?></td>
                            <td><?= htmlspecialchars($res['grade']) ?></td>
                            <td><?= htmlspecialchars($res['session']) ?></td>
                            <td><?= htmlspecialchars($res['semester']) ?></td>
                            <td>
                                <a href="approve_result_action.php?id=<?= $res['id'] ?>" class="btn btn-success btn-sm btn-action"><i class="fa fa-check"></i> Approve</a>
                                <a href="reject_result_action.php?id=<?= $res['id'] ?>" class="btn btn-danger btn-sm btn-action"><i class="fa fa-times"></i> Reject</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="8" class="text-center text-muted">No results pending approval.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
