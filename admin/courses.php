...
try {
    $stmt = $pdo->query("SELECT id, course_code, course_title, unit_load, department_id FROM courses ORDER BY id DESC");
    $courses = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch departments for mapping department names
    $stmtDept = $pdo->query("SELECT id, department_name FROM departments");
...
.sidebar h3 { color:#fff; font-size:1.3rem; margin-bottom:30px; text-align:left; }
.sidebar a { color:#fff; text-decoration:none; display:flex; align-items:center; padding:10px 15px; margin-bottom:5px; border-radius:5px; }
.sidebar a i { margin-right:10px; font-size:16px; }
.sidebar a:hover, .sidebar a.active { background: rgba(255,255,255,0.14); }
.sidebar-divider { height:1px; background: rgba(255,255,255,0.14); margin:20px 0; }
...
<div class="sidebar">
    ...
    <a href="courses.php" class="active"><i class="fa fa-book"></i> Courses</a>
...
<div class="content">
    <h2>Courses</h2>
    <p>Below is a list of all courses. You can view, edit, or delete a course record.</p>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= htmlspecialchars($deptMap[$course['department_id']] ?? 'Not Set') ?></td>
                    <td>
                        <a href="view_course.php?id=<?= $course['id'] ?>" class="btn btn-info btn-sm"><i class="fa fa-eye"></i> View</a>
                        <a href="edit_course.php?id=<?= $course['id'] ?>" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</a>
                        <a href="delete_course.php?id=<?= $course['id'] ?>" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
